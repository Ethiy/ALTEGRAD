...
train_proj=pca.transform(trainingData)
clf = svm.SVC()
clf.fit(train_proj, trainingLabels)
...
projectedtest=pca.transform(testData);

y=clf.predict(projectedtest)
...
print 'Accuracy of PCA: %f' % (counter2 / float(y.size) * 100.0)
