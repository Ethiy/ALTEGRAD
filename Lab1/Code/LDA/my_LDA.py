...
def my_LDA(X, Y):
    ...
    # ========= Indentifying the Classes

    classes = {classLabels[i]: X[Y==classLabels[i]] for i in range(classNum) };

    # ========= The Centroids

    means = { i: np.mean(classes[ i ],axis=0) for i in classes.keys() };

    # ========= The within class scatter matrix

    centered = {i:classes[ i ]-means[i]  for i in classes.keys() };

    Sw = sum ([ sum([np.dot(centered[ i ][j,:][np.newaxis,:].T , centered[ i ][j,:][np.newaxis,:] ) for j in range(len(centered[i])) ] )  for i in classes.keys() ]);
    
    # ========= The between class scater matrix
    
    m = np.mean(X,axis=0);

    Sb = sum( [ len(classes[ i ]) * np.dot( (means[ i ] - m)[np.newaxis,:].T , (means[ i ] - m)[np.newaxis,:] ) for i in classes.keys() ]);
    ...
    # ========= Projecting in the new space

    X_lda = np.dot( X , W );

    projected_centroid = np.dot( np.array( [ means[i] for i in means.keys() ] ) , W );
